...
//assume byteIndex in range
int byteToCharIndex(const std::string& s, int byteIndex) {
    int charIndex = -1;

    for (int currentByteIndex=0; currentByteIndex <= byteIndex; ++currentByteIndex) {
        if (isStartByte(s[currentByteIndex])) {
            ++charIndex;
        }
    }
    return charIndex;
...
//assume byte from valid utf8 string
bool isStartByte(unsigned char byte) {
    return ((byte & 0xC0) != 0x80);
    /*
    return (byte & 0x80) == 0x00             // 1-byte character: starts with 0
           || (byte & 0xE0) == 0xC0          // 2-byte character: starts with 110
           || (byte & 0xF0) == 0xE0          // 3-byte character: starts with 1110
           || (byte & 0xF8) == 0xF0;         // 4-byte character: starts with 11110
    */
}

//assumes s only has single width chars
void overwriteColWithU8(std::string &s, unsigned int col, std::string stuff) {
    int stuffWidth = stringWidth(stuff);
    std::string s1 = substrU8(s, 0, col);
    std::string s2 = substrU8(s, col+stuffWidth, lengthU8(s)-col-stuffWidth);
    s = substrU8(s, 0, col) + stuff + substrU8(s, col+stuffWidth, lengthU8(s)-col-stuffWidth);
}
